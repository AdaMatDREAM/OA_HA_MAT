...
# Max flow problemet er LP (pga. totally unimodular matrix)
if P.model_type == "LP"
    if P.output_terminal
        ...
        if status_str == "OPTIMAL" || status_str == "ALMOST_OPTIMAL"
            ...
            println("─"^100)
            if all_integer
                println("   Success: Alle løsningsværdier er heltallige")
    ...
    error("Max flow problemet skal være LP (model_type = \"LP\")")
end
