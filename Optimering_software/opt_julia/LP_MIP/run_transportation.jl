...
include("Transportation_problem_skabelon.jl")

# Indlæs modellen og indstillinger
# Syntax P.[variabelnavn]
P = Transportation_problem_skabelon()
...
# Transportation problemet kan være enten LP eller MIP
if P.model_type == "MIP"
    if P.output_terminal
        ...
        # Print transportation matrix hvis optimal løsning (modellen er allerede optimeret i standard_MIP_output)
        status = termination_status(M)
        status_str = string(status)
        if status_str == "OPTIMAL" || status_str == "ALMOST_OPTIMAL"
            print_assignment_transportation_matrix(M, x, P.x_navne, P.supply_navne, P.demand_navne, P.dec)
        end
    end
...
elseif P.model_type == "LP"
    if P.output_terminal
        ...
        # Print transportation matrix hvis optimal løsning (modellen er allerede optimeret i standard_LP_output)
        status = termination_status(M)
        status_str = string(status)
        if status_str == "OPTIMAL" || status_str == "ALMOST_OPTIMAL"
            print_assignment_transportation_matrix(M, x, P.x_navne, P.supply_navne, P.demand_navne, P.dec)
    ...
    error("Transportation problemet skal være enten LP eller MIP (model_type = \"LP\" eller \"MIP\")")
end
